...
export const updateUserProfile = async (req, res) => {
    ...
    const { password, ...userInfoWithoutPassword } = user;
    return new SuccessResponse(StatusCodes.OK, userInfoWithoutPassword, "User profile updated").send(res);
};
export const updateUserAvtarImg = async (req, res) => {
    ...
    const avatarImgURL = await AwsUploadService.uploadFileWithContent(`${findUser.userId}-${files?.avatarImg?.name}`, files?.avatarImg?.data);
    const user = await prisma.user.update({
        data: {
